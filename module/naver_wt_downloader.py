...
import numpy as np
from bst_down.naver_webtoon.module_in_wt.module import etc, image_edit
...
def downloader(ls):
    for lang in ls:
        ...
        adap, ider, weekday = etc.get_id_wk(lang)
        Soup1 = etc.get_soup(adap)
        ...
        for epsoide in brr1:
            ...
            number += 1
            img = image_edit.img_add(img_arr)
            n = 1
            cut_img_y = 0
            pix = np.array(img)
            counter = 0
                ...
                if counter == 300:
                    ...
                    if int(cut_img.size[1]) <= 300:
                        ...
                        if cut_img.size[1] >= 50000:
                            ...
                            n += 1
                            image_edit.overpx_edit(cut_img, basic_path, base_folder, eposide_title, la, n)
                        else:
